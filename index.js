...
function manipulateUrl(url) {
  ...
  const pulateUrl = new URL(url);
  pulateUrl.protocol = "https";
  pulateUrl.host = "newhost.com";
  pulateUrl.searchParams.set("newParam", "newValue");
  pulateUrl.searchParams.delete("oldParam");
  return {
    href: pulateUrl.href,
    protocol: pulateUrl.protocol,
    host: pulateUrl.host,
    pathname: pulateUrl.pathname,
    search: pulateUrl.search,
    params: Array.from(pulateUrl.searchParams.entries()),
  };
}
...
function getUrlData(url) {
  ...
  const urlObj = new URL(url);
  const urlData = {};
  urlData.origin = urlObj.origin;
  urlData.hostname = urlObj.hostname;
  urlData.port = urlObj.port;
  urlData.username = urlObj.username;
  urlData.password = urlObj.password;
  return urlData;
}
